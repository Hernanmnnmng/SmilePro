...
<div class="container mx-auto py-8 px-4 sm:px-6 lg:px-8">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold">Alle Facturen</h2>
        <a href="{{ route('dashboard') }}" class="text-blue-600 hover:text-blue-800 underline text-sm">← Terug naar Dashboard</a>
    </div>
            ...
            <table class="min-w-full bg-white border">
                <thead>
                    <tr>
                        ...
                        <th class="py-2 px-2 sm:px-4 border-b text-left text-xs sm:text-sm font-semibold">Patiënt</th>
                ...
                <tbody>
                    @foreach($invoices as $invoice)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="py-2 px-2 sm:px-4 border-b text-sm">{{ $invoice->patient->name }}</td>
                            ...
                            <td class="py-2 px-2 sm:px-4 border-b">
                                <div class="flex flex-col sm:flex-row gap-2">
                                    ...
                                    <form method="POST" action="{{ route('invoices.destroy', $invoice->id) }}" class="inline" onsubmit="return confirm('Weet je zeker dat je factuur {{ $invoice->invoice_number }} wilt verwijderen? Deze actie kan niet ongedaan worden gemaakt.');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded text-sm whitespace-nowrap w-full sm:w-auto">
                                            Verwijder
                                        </button>
                                    </form>
                                </div>
    ...
    @else
        <div class="bg-gray-100 border border-gray-300 rounded p-6 text-center">
            ...
            <p class="text-gray-500 text-sm mt-2">Er zijn nog geen facturen aangemaakt.</p>
        </div>
    @endif
</div>
@endsection
